...
class _ActivityRow extends StatelessWidget {
  ...
  String _name(String uid) {
    final currentName = names[uid]?.trim() ?? '';
    return currentName.isEmpty ? uid : currentName;
  }

  String _description() {
    final actor = _name(entry.actorUid);
    final targetUid = entry.targetUid;
    final target = targetUid == null ? '' : _name(targetUid);
    ...
    return switch (entry.action) {
      ...
      'member.role_changed' => 'activity_action_member_role_changed'.tr(
        namedArgs: {
          ...arguments,
          'from': _role(entry.before['role']),
          'to': _role(entry.after['role']),
        },
      ),
      'member.banned' => 'activity_action_member_banned'.tr(
        namedArgs: arguments,
      ),
      ...
      'company.join_policy_changed' => 'activity_action_join_policy_changed'.tr(
        namedArgs: {
          'actor': actor,
          'policy': _joinPolicy(entry.after['joinPolicy']),
        },
      ),
  ...
  String _joinPolicy(Object? value) => switch (value) {
