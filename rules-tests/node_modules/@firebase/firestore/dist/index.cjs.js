 ...
 */ let Jt = class DocumentSnapshot {
     ...
     */    get ref() {
        return new common.X(this._firestore, this._converter, this._key);
    }
    /**
    ...
    get(t) {
        if (this._document) {
            const e = this._document.data.field(common.ta("DocumentSnapshot.get", t));
...
function __PRIVATE_validateHasExplicitOrderByForLimitToLast(t) {
    if ("L" /* LimitType.Last */ === t.limitType && 0 === t.explicitOrderBy.length) throw new common.s(common.aa.UNIMPLEMENTED, "limitToLast() queries require specifying at least one orderBy() clause");
}
...
function query(t, e, ...n) {
    ...
    e instanceof AppliableConstraint && r.push(e), r = r.concat(n), function __PRIVATE_validateQueryConstraintArray(t) {
        ...
        if (e > 1 || e > 0 && n > 0) throw new common.s(common.aa.INVALID_ARGUMENT, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
    }
    /**
 * @license
 * Copyright 2020 Google LLC
 ...
 */ class QueryFieldFilterConstraint extends QueryConstraint {
    ...
    _parse(t) {
        const e = common.la(t.firestore), n = function __PRIVATE_newQueryFilter(t, e, n, r, s, o, i) {
            let a;
            if (s.isKeyField()) {
                if ("array-contains" /* Operator.ARRAY_CONTAINS */ === o || "array-contains-any" /* Operator.ARRAY_CONTAINS_ANY */ === o) throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid Query. You can't perform '${o}' queries on documentId().`);
            ...
            a = common.__PRIVATE_parseQueryValue(n, e, i, 
            /* allowArrays= */ "in" /* Operator.IN */ === o || "not-in" /* Operator.NOT_IN */ === o);
            const u = common.FieldFilter.create(s, o, a);
            return u;
 ...
 */ class QueryOrderByConstraint extends QueryConstraint {
    ...
    _apply(t) {
        const e = function __PRIVATE_newQueryOrderBy(t, e, n) {
            if (null !== t.startAt) throw new common.s(common.aa.INVALID_ARGUMENT, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
            if (null !== t.endAt) throw new common.s(common.aa.INVALID_ARGUMENT, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
 ...
 */ function orderBy(t, e = "asc") {
    const n = e, r = common.ta("orderBy", t);
...
/** Helper function to create a bound from a document or fields */ function __PRIVATE_newQueryBoundFromDocOrFields(t, e, n, r) {
    if (n[0] = util.getModularInstance(n[0]), n[0] instanceof Jt) return function __PRIVATE_newQueryBoundFromDocument(t, e, n, r, s) {
        if (!r) throw new common.s(common.aa.NOT_FOUND, `Can't use a DocumentSnapshot that doesn't exist for ${n}().`);
        const o = [];
        ...
        // results.
                for (const n of common.__PRIVATE_queryNormalizedOrderBy(t)) if (n.field.isKeyField()) o.push(common.__PRIVATE_refValue(e, r.key)); else {
            const t = r.data.field(n.field);
            if (common.__PRIVATE_isServerTimestamp(t)) throw new common.s(common.aa.INVALID_ARGUMENT, 'Invalid query. You are trying to start or end a query using a document for which the field "' + n.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
            if (null === t) {
                const t = n.field.canonicalString();
                throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. You are trying to start or end a query using a document for which the field '${t}' (used as the orderBy) does not exist.`);
            }
            o.push(t);
        }
        return new common.Bound(o, s);
    }
    /**
 * Converts a list of field values to a `Bound` for the given query.
 */ (t._query, t.firestore._databaseId, e, n[0]._document, r);
    {
        const s = common.la(t.firestore);
        return function __PRIVATE_newQueryBoundFromFields(t, e, n, r, s, o) {
            // Use explicit order by's because it has to match the query the user made
            const i = t.explicitOrderBy;
            if (s.length > i.length) throw new common.s(common.aa.INVALID_ARGUMENT, `Too many arguments provided to ${r}(). The number of arguments must be less than or equal to the number of orderBy() clauses`);
            const a = [];
            for (let o = 0; o < s.length; o++) {
                const u = s[o];
                if (i[o].field.isKeyField()) {
                    if ("string" != typeof u) throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. Expected a string for document ID in ${r}(), but got a ${typeof u}`);
                    if (!common.__PRIVATE_isCollectionGroupQuery(t) && -1 !== u.indexOf("/")) throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. When querying a collection and ordering by documentId(), the value passed to ${r}() must be a plain document ID, but '${u}' contains a slash.`);
                    const n = t.path.child(common.ResourcePath.fromString(u));
                    if (!common.DocumentKey.isDocumentKey(n)) throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. When querying a collection group and ordering by documentId(), the value passed to ${r}() must result in a valid document path, but '${n}' is not because it contains an odd number of segments.`);
                    const s = new common.DocumentKey(n);
                    a.push(common.__PRIVATE_refValue(e, s));
                } else {
                    const t = common.__PRIVATE_parseQueryValue(n, r, u);
                    a.push(t);
                }
            }
            return new common.Bound(a, o);
        }
        /**
...
function __PRIVATE_parseDocumentIdValue(t, e, n) {
    if ("string" == typeof (n = util.getModularInstance(n))) {
        if ("" === n) throw new common.s(common.aa.INVALID_ARGUMENT, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
        if (!common.__PRIVATE_isCollectionGroupQuery(e) && -1 !== n.indexOf("/")) throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${n}' contains a '/' character.`);
        const r = e.path.child(common.ResourcePath.fromString(n));
        if (!common.DocumentKey.isDocumentKey(r)) throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${r}' is not because it has an odd number of segments (${r.length}).`);
        return common.__PRIVATE_refValue(t, new common.DocumentKey(r));
    }
    if (n instanceof common.X) return common.__PRIVATE_refValue(t, n._key);
    throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. When querying with documentId(), you must provide a valid string or a DocumentReference, but it was: ${common.__PRIVATE_valueDescription(n)}.`);
}
 ...
 */ function __PRIVATE_validateDisjunctiveFilterElements(t, e) {
    if (!Array.isArray(t) || 0 === t.length) throw new common.s(common.aa.INVALID_ARGUMENT, `Invalid Query. A non-empty array is required for '${e.toString()}' filters.`);
}
 ...
 */ function __PRIVATE_validateNewFieldFilter(t, e) {
    ...
    throw n === e.op ? new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. You cannot use more than one '${e.op.toString()}' filter.`) : new common.s(common.aa.INVALID_ARGUMENT, `Invalid query. You cannot use '${e.op.toString()}' filters with '${n.toString()}' filters.`);
}

function __PRIVATE_validateQueryFilterConstraint(t, e) {
    if (!(e instanceof QueryFieldFilterConstraint || e instanceof QueryCompositeFilterConstraint)) throw new common.s(common.aa.INVALID_ARGUMENT, `Function ${t}() requires AppliableConstraints created with a call to 'where(...)', 'or(...)', or 'and(...)'.`);
}
...
class __PRIVATE_LiteUserDataWriter extends common.AbstractUserDataWriter {
    ...
    convertReference(t) {
        ...
        return new common.X(this.firestore, /* converter= */ null, e);
    }
}
 ...
 */ function sum(t) {
    return new AggregateField("sum", common.ta("sum", t));
}
 ...
 */ function average(t) {
    return new AggregateField("avg", common.ta("average", t));
}
 ...
 */ function getAggregateFromServer(t, e) {
    const n = common.ra(t.firestore, common.da), r = common.oa(n), o = common.__PRIVATE_mapToArray(e, ((t, e) => new common.__PRIVATE_AggregateImpl(e, t.aggregateType, t._internalFieldPath)));
    // Run the aggregation and convert the results
    return common.__PRIVATE_firestoreClientRunAggregateQuery(r, t._query, o).then((e => 
    /**
 ...
 */
    function __PRIVATE_convertToAggregateQuerySnapshot(t, e, n) {
        const r = new common.ua(t), s = new AggregateQuerySnapshot(e, r, n);
        return s;
    }
    /**
 * @license
 ...
 */ class DocumentSnapshot extends Jt {
    ...
    get(t, e = {}) {
        if (this._document) {
            const n = this._document.data.field(common.ta("DocumentSnapshot.get", t));
     ...
     */    toJSON() {
        if (this.metadata.hasPendingWrites) throw new common.s(common.aa.FAILED_PRECONDITION, "DocumentSnapshot.toJSON() attempted to serialize a document with pending writes. Await waitForPendingWrites() before invoking toJSON().");
...
function documentSnapshotFromJSON(t, e, n) {
    if (common.__PRIVATE_validateJSON(e, DocumentSnapshot._jsonSchema)) {
        if (e.bundle === Gt) throw new common.s(common.aa.INVALID_ARGUMENT, "The provided JSON object was created in a client environment, which is not supported.");
        // Parse the bundle data.
                const r = common.__PRIVATE_newSerializer(t._databaseId), s = common.__PRIVATE_createBundleReaderSync(e.bundle, r), o = s.va(), i = new common.__PRIVATE_BundleLoader(s.getMetadata(), r);
        for (const t of o) i.qu(t);
        // Ensure that we have the correct number of documents in the bundle.
                const a = i.documents;
        if (1 !== a.length) throw new common.s(common.aa.INVALID_ARGUMENT, `Expected bundle data to contain 1 document, but it contains ${a.length} documents.`);
        // Build out the internal document data.
                const u = common.fromDocument(r, a[0].document), c = new common.DocumentKey(common.ResourcePath.fromString(e.bundleName));
 ...
 */ class QuerySnapshot {
     ...
     */    docChanges(t = {}) {
        ...
        if (e && this._snapshot.excludesMetadataChanges) throw new common.s(common.aa.INVALID_ARGUMENT, "To include metadata changes with your document changes, you must also pass { includeMetadataChanges:true } to onSnapshot().");
        ...
        function __PRIVATE_changesFromSnapshot(t, e) {
            if (t._snapshot.oldDocs.isEmpty()) {
                ...
                return t._snapshot.docChanges.map((n => {
                    common.__PRIVATE_isPipeline(t._snapshot.query) ? common.__PRIVATE_newPipelineComparator(t._snapshot.query) : common.__PRIVATE_newQueryComparator(t.query._query);
     ...
     */    toJSON() {
        if (this.metadata.hasPendingWrites) throw new common.s(common.aa.FAILED_PRECONDITION, "QuerySnapshot.toJSON() attempted to serialize a document with pending writes. Await waitForPendingWrites() before invoking toJSON().");
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const t = {};
...
function querySnapshotFromJSON(t, e, n) {
    if (common.__PRIVATE_validateJSON(e, QuerySnapshot._jsonSchema)) {
        ...
        // Parse the bundle data.
                const r = common.__PRIVATE_newSerializer(t._databaseId), s = common.__PRIVATE_createBundleReaderSync(e.bundle, r), o = s.va(), i = new common.__PRIVATE_BundleLoader(s.getMetadata(), r);
        for (const t of o) i.qu(t);
        if (1 !== i.queries.length) throw new common.s(common.aa.INVALID_ARGUMENT, `Snapshot data expected 1 query but found ${i.queries.length} queries.`);
        // Create an internal Query object from the named query in the bundle.
                const a = common.__PRIVATE_fromBundledQuery(i.queries[0].bundledQuery), u = common.__PRIVATE_isPipeline(a) ? common.__PRIVATE_newPipelineComparator(a) : common.__PRIVATE_newQueryComparator(a), c = i.documents;
        let l = new common.DocumentSet(u);
        c.map((t => {
            const e = common.fromDocument(r, t.document);
            l = l.add(e);
        }));
        // Create a view snapshot of the query and documents.
        const h = common.ViewSnapshot.fromInitialDocuments(a, l, common.__PRIVATE_documentKeySet() /* Zero mutated keys signifies no pending writes. */ , 
        /* fromCache= */ false, 
        /* hasCachedResults= */ false), d = new common.Query(t, n || null, a);
...
class WriteBatch {
    ...
    constructor(t, e) {
        ...
        this._dataReader = common.la(t);
    }
    set(t, e, n) {
        this._verifyNotCommitted();
        const r = __PRIVATE_validateReference(t, this._firestore), s = __PRIVATE_applyFirestoreDataConverter(r.converter, e, n), o = common.__PRIVATE_parseSetData(this._dataReader, "WriteBatch.set", r._key, s, null !== r.converter, n);
        return this._mutations.push(o.toMutation(r._key, common.Precondition.none())), this;
    }
    update(t, e, n, ...r) {
        this._verifyNotCommitted();
        ...
        // performing validation.
                let o;
        return o = "string" == typeof (e = util.getModularInstance(e)) || e instanceof common.FieldPath ? common.__PRIVATE_parseUpdateVarargs(this._dataReader, "WriteBatch.update", s._key, e, n, r) : common.__PRIVATE_parseUpdateData(this._dataReader, "WriteBatch.update", s._key, e), 
        this._mutations.push(o.toMutation(s._key, common.Precondition.exists(true))), this;
    }
    /**
    ...
    _verifyNotCommitted() {
        if (this._committed) throw new common.s(common.aa.FAILED_PRECONDITION, "A write batch can no longer be used after commit() has been called.");
    }
}

function __PRIVATE_validateReference(t, e) {
    if ((t = util.getModularInstance(t)).firestore !== e) throw new common.s(common.aa.INVALID_ARGUMENT, "Provided document reference is from a different Firestore instance.");
    return t;
}
 ...
 */ let Ut = class Transaction {
    /** @hideconstructor */
    constructor(t, e) {
        this._firestore = t, this._transaction = e, this._dataReader = common.la(t);
    }
    /**
     ...
     */    get(t) {
        ...
        return this._transaction.lookup([ e._key ]).then((t => {
            if (!t || 1 !== t.length) return common.l(24041);
            const r = t[0];
    ...
    set(t, e, n) {
        const r = __PRIVATE_validateReference(t, this._firestore), s = __PRIVATE_applyFirestoreDataConverter(r.converter, e, n), o = common.__PRIVATE_parseSetData(this._dataReader, "Transaction.set", r._key, s, null !== r.converter, n);
    ...
    update(t, e, n, ...r) {
        ...
        // performing validation.
                let o;
        return o = "string" == typeof (e = util.getModularInstance(e)) || e instanceof common.FieldPath ? common.__PRIVATE_parseUpdateVarargs(this._dataReader, "Transaction.update", s._key, e, n, r) : common.__PRIVATE_parseUpdateData(this._dataReader, "Transaction.update", s._key, e), 
 ...
 */ class Transaction extends Ut {
     ...
     * @returns A `DocumentSnapshot` with the read data.
     */    get(t) {
        const e = __PRIVATE_validateReference(t, this._firestore), n = new common.ua(this._firestore);
        return super.get(t).then((t => new DocumentSnapshot(this._firestore, n, e._key, t._document, new SnapshotMetadata(
        /* hasPendingWrites= */ false, 
        /* fromCache= */ false), e.converter)));
    }
}
 ...
 */ function runTransaction(t, e, n) {
    t = common.ra(t, common.da);
    const r = {
        ...jt,
        ...n
    };
    !function __PRIVATE_validateTransactionOptions(t) {
        if (t.maxAttempts < 1) throw new common.s(common.aa.INVALID_ARGUMENT, "Max attempts must be at least 1");
    }(r);
    const o = common.oa(t);
    return common.__PRIVATE_firestoreClientTransaction(o, (n => e(new Transaction(t, n))), r);
}
 ...
 * document contents.
 */ function getDoc(t) {
    t = common.ra(t, common.X);
    const e = common.ra(t.firestore, common.da), n = common.oa(e);
 ...
 */ function getDocFromCache(t) {
    t = common.ra(t, common.X);
    const e = common.ra(t.firestore, common.da), n = common.oa(e), r = new common.ua(e);
...
function setDoc(t, e, n) {
    t = common.ra(t, common.X);
    const r = common.ra(t.firestore, common.da), o = __PRIVATE_applyFirestoreDataConverter(t.converter, e, n), i = common.la(r);
    return executeWrite(r, [ common.__PRIVATE_parseSetData(i, "setDoc", t._key, o, null !== t.converter, n).toMutation(t._key, common.Precondition.none()) ]);
}

function updateDoc(t, e, n, ...r) {
    t = common.ra(t, common.X);
    const o = common.ra(t.firestore, common.da), i = common.la(o);
    let a;
    a = "string" == typeof (
    // For Compat types, we have to "extract" the underlying types before
    // performing validation.
    e = util.getModularInstance(e)) || e instanceof common.FieldPath ? common.__PRIVATE_parseUpdateVarargs(i, "updateDoc", t._key, e, n, r) : common.__PRIVATE_parseUpdateData(i, "updateDoc", t._key, e);
    return executeWrite(o, [ a.toMutation(t._key, common.Precondition.exists(true)) ]);
}
 ...
 */ function deleteDoc(t) {
    return executeWrite(common.ra(t.firestore, common.da), [ new common.__PRIVATE_DeleteMutation(t._key, common.Precondition.none()) ]);
}
 ...
 * the document.
 */ function addDoc(t, e) {
    const n = common.ra(t.firestore, common.da), r = common.doc(t), o = __PRIVATE_applyFirestoreDataConverter(t.converter, e), i = common.la(t.firestore);
    return executeWrite(n, [ common.__PRIVATE_parseSetData(i, "addDoc", r._key, o, null !== t.converter, {}).toMutation(r._key, common.Precondition.exists(false)) ]).then((() => r));
}
...
function onSnapshot(t, ...e) {
    ...
    let i, a, u;
    if (t instanceof common.X) a = common.ra(t.firestore, common.da), u = common.__PRIVATE_newQueryForPath(t._key.path), i = {
        next: n => {
        ...
        const n = common.ra(t, common.Query);
        a = common.ra(n.firestore, common.da), u = n._query;
        const o = new common.ua(a);
        i = {
            next: t => {
    ...
    const c = common.oa(a);
    return common.__PRIVATE_firestoreClientListen(c, u, o, i);
}
 ...
 * @internal
 */ (e);
    if (s.error) throw new common.s(common.aa.INVALID_ARGUMENT, s.error);
    let o, i = 0;
 ...
 */
        return function __PRIVATE_onSnapshotQuerySnapshotBundle(t, e, n, r, s) {
            let o, i = false;
            const a = common.loadBundle(t, e.bundle);
            return a.then((() => common.namedQuery(t, e.bundleName))).then((t => {
                if (t && !i) {
 ...
 */ (r, s, o, t, n[i]);
    ...
    if ("DocumentSnapshot" === s.bundleSource) {
        ...
        return function __PRIVATE_onSnapshotDocumentSnapshotBundle(t, e, n, r, s) {
            ...
            return a.then((() => {
                if (!i) {
                    const i = new common.X(t, s || null, common.DocumentKey.fromPath(e.bundleName));
                    o = onSnapshot(i, n || {}, r);
                }
            })).catch((t => (r.error && r.error(t), () => {}))), () => {
                i || (i = true, o && o());
    ...
    throw new common.s(common.aa.INVALID_ARGUMENT, `unsupported bundle source: ${s.bundleSource}`);
}

function onSnapshotsInSync(t, e) {
    t = common.ra(t, common.da);
    const n = common.oa(t), r = __PRIVATE_isPartialObserver(e) ? e : {
        next: e
    };
 ...
 */ function __PRIVATE_convertToDocSnapshot(t, e, n) {
    const r = n.docs.get(e._key), s = new common.ua(t);
...
function writeBatch(t) {
    return t = common.ra(t, common.da), common.oa(t), new WriteBatch(t, (e => executeWrite(t, e)));
}
    ...
    const r = function __PRIVATE_parseIndexes(t) {
        const e = "string" == typeof t ? function __PRIVATE_tryParseJson(t) {
            try {
                ...
                throw new common.s(common.aa.INVALID_ARGUMENT, "Failed to parse JSON: " + t?.message);
            }
        }(t) : t, n = [];
...
function __PRIVATE_tryGetString(t, e) {
    if ("string" != typeof t[e]) throw new common.s(common.aa.INVALID_ARGUMENT, "Missing string value for: " + e);
    return t[e];
}
 ...
 */ function deleteAllPersistentCacheIndexes(t) {
    const e = common.oa(t._firestore);
...
function __PRIVATE_setPersistentCacheIndexAutoCreationEnabled(t, e) {
    const n = common.oa(t._firestore);
 ...
 */ class __PRIVATE_TestingHooksSpiImpl {
    ...
    static get instance() {
        return Yt || (Yt = new __PRIVATE_TestingHooksSpiImpl, common.__PRIVATE_setTestingHooksSpi(Yt)), Yt;
    }
    Ie(t) {
...
function __PRIVATE_registerFirestore(c, l = true) {
    common.__PRIVATE_setSDKVersion(app.SDK_VERSION), app._registerComponent(new component.Component("firestore", ((t, {instanceIdentifier: e, options: n}) => {
        const r = t.getProvider("app").getImmediate(), u = new common.da(new common.__PRIVATE_FirebaseAuthCredentialsProvider(t.getProvider("auth-internal")), new common.__PRIVATE_FirebaseAppCheckTokenProvider(r, t.getProvider("app-check-internal")), common.__PRIVATE_databaseIdFromApp(r, e), r);
        return n = {
            useFetchStreams: l,
            ...n
        }, u._setSettings(n), u;
    }), "PUBLIC").setMultipleInstances(true)), app.registerVersion(Wt, $t, c), 
    // BUILD_TARGET will be replaced by values like esm, cjs, etc during the compilation
    app.registerVersion(Wt, $t, "cjs2020");
}();

exports.AbstractUserDataWriter = common.AbstractUserDataWriter;
exports.Bytes = common.Bytes;
exports.CACHE_SIZE_UNLIMITED = common.pr;
exports.CollectionReference = common.ea;
exports.DocumentReference = common.X;
exports.FieldPath = common.FieldPath;
exports.FieldValue = common.FieldValue;
exports.Firestore = common.da;
exports.FirestoreError = common.s;
